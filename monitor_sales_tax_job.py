#!/usr/bin/env python3
"""
Monitor Sales Tax Processor job progress and status.
...
def display_job_info(job: Job, rq_status=None, watch_mode=False):
    ...
    print("\n" + "=" * 80)
    print(f"Sales Tax Processor Job #{job.id}")
    print("=" * 80)
    ...
    if job.options:
        ...
        rows_processed = job.options.get('rows_processed', 0)
        total_rows = job.options.get('total_rows', 0)
        status_message = job.options.get('status_message', '')

        if total_rows > 0:
            print(f"\nProgress: {progress}%")
            print(f"Rows: {rows_processed:,} / {total_rows:,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Monitor Sales Tax Processor job')
    ...
    try:
        ...
        if args.job_id:
            job = db.query(Job).filter(
                Job.id == args.job_id,
                Job.feature == "sales_tax_processor"
            ).first()
            if not job:
            ...
            if not job:
                print("ERROR: No Sales Tax Processor jobs found")
                sys.exit(1)
