                    ...
                    <div class="upload-options">
                        <div class="option-group">
                            ...
                            <InputText
                                ...
                                placeholder="e.g., OrderID, order_id"
                                class="w-full"
                            />
                        </div>
                        ...
                        <Button
                            ...
                            :disabled="!selectedFile || !orderIdHeader || processing"
                            :loading="processing"
                            type="submit"
                        />
                    </form>
                            ...
                            <div class="job-info">
                                ...
                                <div class="job-details">
                                    <div class="job-filename">
                                        {{ getFileName(job.input_filename) }}
                                    </div>
                                    <div class="job-meta">
...
import { uploadCsv, listJobs, downloadJob, deleteJob } from "../api/salesTaxApi";
...
async function handleUpload() {
    if (!selectedFile.value || !orderIdHeader.value) return;

    processing.value = true;
    try {
        const result = await uploadCsv(selectedFile.value, orderIdHeader.value, options.value);
        toast.add({
            severity: "success",
            summary: "File Uploaded",
            detail: "Processing has started",
            life: 3000,
        });
...
function getProgress(job) {
    ...
    if (job.status === "running" || job.status === "pending") {
        ...
        // Fallback: show minimal progress for pending, small progress for running
...
function getProgressText(job) {
    ...
    if (job.status === "running" || job.status === "pending") {
        // Show status message if available (e.g., "Fetching orders...")
        ...
        if (job.options && typeof job.options.rows_processed === "number" && typeof job.options.total_rows === "number") {
            const processed = job.options.rows_processed;
            const total = job.options.total_rows;
            const percent = job.options.progress || 0;
            return `Processing... ${processed}/${total} (${percent}%)`;
        }
...
async function handleDownload(jobId) {
    try {
        ...
        toast.add({
            ...
            detail: "File download has started",
            life: 3000,
        });
    } catch (error) {
        toast.add({
...
function confirmDelete(job) {
    ...
    const message = isActive
        ? `Are you sure you want to delete this job? This will stop the processing and permanently delete the job and all associated files (${getFileName(job.input_filename)}).`
        : `Are you sure you want to delete this job? This will permanently delete the job and all associated files (${getFileName(job.input_filename)}).`;
...
onMounted(async () => {
    await loadJobs();
    // Refresh jobs every 2 seconds for more responsive updates
