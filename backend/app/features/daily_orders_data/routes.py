...
from .worker import run_daily_orders_export_job
from .scheduler_service import schedule_rq_job, unschedule_rq_job

router = APIRouter(
    prefix="/api/app/daily-orders-data",
    tags=["daily-orders-data"],
)

settings = get_settings()


@router.post("/export", response_model=schemas.DailyOrdersExportResponse)
def create_export(
    request: schemas.DailyOrdersExportRequest,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    Create a new Daily Orders Data export job (manual run).
    """
    ...
    # Enqueue the job
    enqueue_job(
        run_daily_orders_export_job,
        job.id,
        job_timeout=3600  # 1 hour timeout
    )

    return schemas.DailyOrdersExportResponse(
        job_id=job.id,
        message="Export job created successfully"
    )


@router.get("/jobs", response_model=List[schemas.DailyOrdersJobStatus])
def list_jobs(
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    List all Daily Orders Data export jobs.
    """
    jobs = db.query(Job).filter(Job.feature == "daily_orders_data").order_by(Job.created_at.desc()).limit(100).all()
    return jobs


@router.get("/jobs/{job_id}", response_model=schemas.DailyOrdersJobStatus)
def get_job(
    job_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    ...
    """
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "daily_orders_data").first()
    if not job:
...
def delete_job(
    ...
    """
    Delete a Daily Orders Data export job and its associated files.
    """
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "daily_orders_data").first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def get_scheduler_status_endpoint(
    ...
    try:
        ...
        scheduled_export = db.query(ScheduledExport).filter(
            ScheduledExport.feature == "daily_orders_data",
