...
def create_scheduled_export_job(scheduled_export_id: int):
    ...
    For daily orders, it exports the previous day's data.
    """
    ...
    try:
        ...
        # Calculate date for export (previous calendar day in America/New_York timezone, like Ulta)
        # This matches the timezone used by WooCommerce/Metorik (America/New_York)
        ...
        # Export the full previous calendar day: 00:00:00 to 23:59:59.999999 in NY timezone
        ...
        # Convert to UTC for the API (worker expects UTC)
        yesterday_start_utc = yesterday_start.astimezone(ZoneInfo("UTC"))

        # Format date as ISO string (worker expects single date, not range)
        # Format: YYYY-MM-DDTHH:mm:ss.sssZ (e.g., "2025-11-01T00:00:00.000Z")
        # Use UTC time for the API
        date_str = yesterday_start_utc.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

        # Format for display (use NY date for display)
        ...
        # Get export options from scheduled export options
        ...
        job = Job(
            ...
            input_filename="",
            options={
                "date": date_str,  # Worker expects "date" not "start_date"/"end_date"
                "is_manual": False,
        ...
        rq_job = queue.enqueue(run_daily_orders_export_job, job.id, job_timeout=3600)
...
def schedule_rq_job(db: Session, scheduled_export: ScheduledExport):
    ...
    try:
        ...
        rq_job_id = f"daily_orders_data_scheduled_export_{scheduled_export.id}"
