...
router = APIRouter(
    prefix="/api/app/tiktok-marketplace",
    tags=["tiktok-marketplace"],
)

settings = get_settings()


@router.post("/export", response_model=schemas.TikTokExportResponse)
def create_export(
    request: schemas.TikTokExportRequest,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    Create a new TikTok Marketplace export job (manual run).
    """
    ...
    # Get Order Desk credentials from environment or settings
    ...
    if not orderdesk_store_id or not orderdesk_api_key:
        raise HTTPException(status_code=500, detail="Order Desk Store ID and API key not configured")
    ...
    # Enqueue the job
    enqueue_job(
        run_tiktok_export_job,
        job.id,
        job_timeout=3600  # 1 hour timeout
    )

    return schemas.TikTokExportResponse(
        job_id=job.id,
        message="Export job created successfully"
    )


@router.get("/jobs", response_model=List[schemas.TikTokJobStatus])
def list_jobs(
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    List all TikTok Marketplace export jobs.
    """
    jobs = db.query(Job).filter(Job.feature == "tiktok_marketplace").order_by(Job.created_at.desc()).limit(100).all()
    return jobs


@router.get("/jobs/{job_id}", response_model=schemas.TikTokJobStatus)
def get_job(
    job_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    ...
    """
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "tiktok_marketplace").first()
    if not job:
...
def delete_job(
    ...
    """
    Delete a TikTok Marketplace export job and its associated files.
    """
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "tiktok_marketplace").first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def get_scheduler_status_endpoint(
    ...
    Returns next run time and last run time from the first enabled scheduled export.
    """
    ...
    try:
        ...
        scheduled_export = db.query(ScheduledExport).filter(
            ScheduledExport.feature == "tiktok_marketplace",
        ...
        if not scheduled_export or not scheduled_export.rq_job_id:
            ...
            last_job = db.query(Job).filter(
                Job.feature == "tiktok_marketplace",
