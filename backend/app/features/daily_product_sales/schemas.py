...
class DailyProductSalesExportRequest(BaseModel):
    ...
    date_from_display: str = None  # Original selected start date (YYYY-MM-DD) for display
    date_to_display: str = None  # Original selected end date (YYYY-MM-DD) for display
    export_to_file: bool = True  # Export to CSV file
    export_to_google_sheets: bool = True  # Export to Google Sheets


class DailyProductSalesExportResponse(BaseModel):
    job_id: int
    message: str


class DailyProductSalesJobStatus(BaseModel):
    id: int
    status: str
    created_at: datetime
    updated_at: datetime
...
# Scheduled Export Schemas (same as daily_orders_data)
