...
def create_scheduled_export_job(scheduled_export_id: int):
    ...
    For daily product sales, it exports the previous day's data (single day range).
    """
    ...
    try:
        ...
        # Calculate date range for export (previous calendar day in America/New_York timezone)
        ...
        rq_job = queue.enqueue(run_daily_product_sales_export_job, job.id, job_timeout=7200)
...
def schedule_rq_job(db: Session, scheduled_export: ScheduledExport):
    ...
    try:
        ...
        # Calculate next run time based on period (same logic as daily_orders_data)
        ...
        rq_job_id = f"daily_product_sales_scheduled_export_{scheduled_export.id}"
