...
@router.post("/export", response_model=schemas.YtInfluencersExportResponse)
def create_export(
    request: schemas.YtInfluencersExportRequest,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """
    Create a new YT Influencers export job (manual run).
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Invalid date format. Use ISO format: YYYY-MM-DDTHH:MM:SSZ. Error: {str(e)}",
        )
    ...
    job = Job(
        ...
        options={
            ...
            "date_display": request.date_display or request.date.split("T")[0],
        },
    )
    db.add(job)
    db.commit()
    ...
    enqueue_job(run_yt_influencers_export_job, job.id, job_timeout=3600)

    return schemas.YtInfluencersExportResponse(
        job_id=job.id,
        message="Export job created successfully",
    )


@router.get("/jobs", response_model=List[schemas.YtInfluencersJobStatus])
def list_jobs(
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """List all YT Influencers export jobs."""
    return (
        db.query(Job)
        .filter(Job.feature == "yt_influencers")
...
@router.get("/jobs/{job_id}", response_model=schemas.YtInfluencersJobStatus)
def get_job(
    job_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """Get details of a specific YT Influencers export job."""
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "yt_influencers").first()
    if not job:
...
def download_job(
    ...
    """Download the CSV file for a completed YT Influencers job."""
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "yt_influencers").first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def delete_job(
    ...
    """Delete a YT Influencers export job and its associated files."""
    job = db.query(Job).filter(Job.id == job_id, Job.feature == "yt_influencers").first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def get_scheduler_status_endpoint(
    ...
    """Get the status of the YT Influencers scheduled export scheduler."""
    ...
    try:
        scheduled_export = db.query(ScheduledExport).filter(
            ScheduledExport.feature == "yt_influencers",
        ...
        return {"scheduler_running": False, "next_run": None, "last_run": None, "error": str(e)}
...
def create_scheduled_export(
    ...
    """Create a new YT Influencers scheduled export configuration."""
    ...
    if request.time:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid time format: {request.time}. Use HH:MM.")
    ...
    return _se_to_dict(scheduled_export)
...
def list_scheduled_exports(
    ...
    """List all YT Influencers scheduled export configurations."""
    exports = (
        db.query(ScheduledExport)
        .filter(ScheduledExport.feature == "yt_influencers")
        .order_by(ScheduledExport.created_at.desc())
        .all()
    )
    return [_se_to_dict(se) for se in exports]
...
def get_scheduled_export(
    ...
    """Get a specific YT Influencers scheduled export configuration."""
...
def update_scheduled_export(
    ...
    """Update a YT Influencers scheduled export configuration."""
    se = db.query(ScheduledExport).filter(
        ScheduledExport.id == scheduled_export_id,
        ScheduledExport.feature == "yt_influencers",
    ).first()
    ...
    if request.time is not None:
        try:
            ...
            se.time = dt_time(int(parts[0]), int(parts[1]) if len(parts) > 1 else 0)
        except Exception:
            raise HTTPException(status_code=400, detail=f"Invalid time format: {request.time}. Use HH:MM.")
        needs_reschedule = True
...
def delete_scheduled_export(
    ...
    """Delete a YT Influencers scheduled export configuration."""
    se = db.query(ScheduledExport).filter(
        ScheduledExport.id == scheduled_export_id,
        ScheduledExport.feature == "yt_influencers",
    ).first()
...
def _se_to_dict(se: ScheduledExport) -> dict:
    return {
        "id": se.id,
        "feature": se.feature,
        "name": se.name,
